...
describe('getRepoRoot', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), 'paths-test-'));
  ...
  it('auto-detects parent .git when own directory has none', () => {
    const host = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), 'host-git-'));
...
describe('getWorkspaceRoot', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), 'ws-test-'));
    for (const k of envKeys) { saved[k] = process.env[k]; delete process.env[k]; }
  });
...
describe('readSessionCwdFromHead', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), 'session-head-'));
  });
